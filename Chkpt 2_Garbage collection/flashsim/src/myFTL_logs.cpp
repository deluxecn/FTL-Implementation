    ...
    public:
        ...
        std::pair<FlashSim::ExecState, FlashSim::Address> write(
            ...
            if (GC_POLICY == 1 || GC_POLICY == 3){
                std::map<size_t, unsigned int>::iterator it_id2ts = id_to_timestamp.find(block_id);
...
//                  << plane << ", " << block << ", " << page << ")\n";
            ...
            if (it == block_map.end()) { // No log block mapped to it.
                if (empty_log_block >= (RAW_CAPACITY - BLOCK_SIZE)) {
                    ...
                    std::cout << "Map a new log-reservation block to the data block: "
                              << data_block_id << "->" << empty_log_block / BLOCK_SIZE << std::endl;
                ...
                std::pair<FlashSim::ExecState, FlashSim::Address> result = logBlock->write(&address);
    ...
    void standardCleaning(size_t data_block_id, LogBlock* log_block, const FlashSim::ExecCallBack<PageType> &func) {
    ...
    void optimizedCleaning(size_t data_block_id, LogBlock* log_block, const FlashSim::ExecCallBack<PageType> &func) {
        size_t data_block_page_start = data_block_id * BLOCK_SIZE;
        FlashSim::Address data_addr = mapping(data_block_page_start);

        func(FlashSim::OpCode::ERASE, data_addr);

        std::map<uint16_t, uint16_t>::iterator it_page = log_block->page_log.begin();
    ...
    std::pair<FlashSim::ExecState, FlashSim::Address> noEmptyPages(
        ...
        if (flag) {
            ...
            if (++clean_erases > BLOCK_ERASES) {
                std::cout << "cleaning page can't erase anymore!\n";
                return std::make_pair(FlashSim::ExecState::FAILURE,
                                      FlashSim::Address(0, 0, 0, 0, 0));
            }
    ...
    std::pair<FlashSim::ExecState, FlashSim::Address> noEmptyLogBlocks(
        ...
        if (GC_POLICY == 0) { // The FIFO policy.
            ...
            std::map<unsigned int, size_t>::iterator it_ts2id = timestamp_to_id.begin();
            ...
            while (ts_it1 != timestamp_to_id.end()) {
                for (it_block_map = block_map.begin(); it_block_map != block_map.end(); it_block_map++) {
                    if (it_block_map->second->block_id == ts_it1->second) {
                        pages_it2 = live_pages.find(it_block_map->first);
                        std::cout << "Found in live_pages!";
                    }
                }
                ...
                ts_it1++;
                std::cout << ratio << "current max_ratio: " << max_ratio << std::endl;
            }
            std::cout << "the max_ratio: " << max_ratio << std::endl;
