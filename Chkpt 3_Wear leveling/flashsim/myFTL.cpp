...
set<size_t> written;
set<size_t> freed_block;
//set<size_t> failed;
class Block;
map<size_t, Block*> logic2data_map;
map<size_t, uint8_t> block_erases;

class Block {
public:
    size_t block_id;
    Block* mapped_block;
    map<uint16_t, uint16_t> page_map;
...
//             << unsigned(BLOCK_ERASES) << endl;
    ...
    ReadTranslate(size_t lba, const FlashSim::ExecCallBack<PageType> &func) {
        ...
        if (data_block->page_map.find(logic_page) != data_block->page_map.end
            ...
            Block* log_block = data_block->mapped_block;
            if (log_block == NULL) {
                cout << "log is null!\n";
            }
    ...
    WriteTranslate(size_t lba, const FlashSim::ExecCallBack<PageType> &func) {
        if (lba >= ADDRESSABLE) {
            return make_pair(FlashSim::ExecState::FAILURE, FlashSim::Address(0, 0, 0, 0, 0));
        }
        size_t logic_block_id = lba / BLOCK_SIZE;
        if (logic2data_map.find(logic_block_id)!=logic2data_map.end()) {
            return writeHasDataBlock(logic2data_map.find(logic_block_id)->second,
                                     lba, func);
        } else {
    ...
    writeNoDataBlock(size_t lba, const FlashSim::ExecCallBack<PageType> &func) {
        ...
        if (data_block == NULL) {
            // No log block.
            cout << "fail: In writeNoDataBlock. No empty "
                    "block found!\n";
            return make_pair(FlashSim::ExecState::FAILURE,
                             FlashSim::Address(0, 0, 0, 0, 0));
        }
    ...
    &func) {
        ...
        if (data_block->empty_page_start == BLOCK_SIZE) {
            ...
            if (data_block->mapped_block != NULL) {
                return writeLog(func, data_block, data_block->mapped_block, lba);
            }
            Block* log = getEmptyBlock(func);
            if (log == NULL) {
                // No log block.
                cout << "fail: In writeHasDataBlock. No empty "
                        "block found!\n";
                return make_pair(FlashSim::ExecState::FAILURE,
                                 FlashSim::Address(0, 0, 0, 0, 0));
            } else {
                data_block->mapped_block = log;
    ...
    pair<FlashSim::ExecState, FlashSim::Address>writeLog
        ...
        if (log->empty_page_start == BLOCK_SIZE) {
            return noEmptyPages(func, data_block, log, lba);
        } else {
            uint8_t logic_page = lba % BLOCK_SIZE;
            data_block->page_map.erase(logic_page);
            ...
            if (data_block->page_map.size() == 0) {
                ...
                cout << "declare invalid: " << data_block->block_id << ". ";
                delete data_block;
            }
            written.insert(lba);
            return make_pair(FlashSim::ExecState::SUCCESS, page2addr
    ...
    pair<FlashSim::ExecState, FlashSim::Address>noEmptyPages
            (const
             FlashSim::ExecCallBack<PageType>
             &func, Block* data_block, Block* log, size_t lba) {
        cout << "Enter noEmptyPages!\n";
        Block* new_block = getEmptyBlock(func);
        uint16_t live_pages = data_block->page_map.size() + log->page_map
                .size();
        cout << "Live pages total: " << live_pages << "\n";
        if (new_block == NULL) {
            cout << "fail: No empty block found. In noEmptyPages!\n";
            return make_pair(FlashSim::ExecState::FAILURE,
                             FlashSim::Address(0, 0, 0, 0, 0));
        } else {
            if (live_pages < WA_LIMIT) {
                return clean(func, data_block, log, new_block, lba);
            } else {
                cout << "fail: Too many live pages. In noEmptyPages!\n";
                return make_pair(FlashSim::ExecState::FAILURE,
                                 FlashSim::Address(0, 0, 0, 0, 0));
            }
        }
    ...
    pair<FlashSim::ExecState, FlashSim::Address>clean(
        ...
        for (page_it = data_block->page_map.begin();
            ...
            func(FlashSim::OpCode::READ, page2addr(data_block->block_id,
                                                   page_it->second));
            func(FlashSim::OpCode::WRITE, page2addr(new_block->block_id,
        ...
        if (written.find(lba) != written.end()) {
            ...
            if (logic2data_map.find(lba / BLOCK_SIZE) != logic2data_map.end())
            {
                ...
                Block* data_block = logic2data_map.find(lba / BLOCK_SIZE)
                        ->second;
                Block* log = data_block->mapped_block;
                ...
                if (log == NULL) {
                    ...
                    if (data_block->page_map.size() == 0
                        ...
                        if (log->page_map.size() == 0
                            ...
                            cout << "declare invalid: " << log->block_id <<
                                 ". ";
                            delete log;
                            data_block->mapped_block = NULL;
                        }
                    }
                }
            }
