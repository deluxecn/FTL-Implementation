...
public:
    ...
    public:
        ...
        std::pair<FlashSim::ExecState, FlashSim::Address> read(
            ...
            std::map<uint16_t, uint16_t>::iterator it = log_page_map.find(addr->page);
        ...
        std::pair<FlashSim::ExecState, FlashSim::Address> write(
                FlashSim::Address *addr, size_t lba) {
...
//        }
        FlashSim::Address address = lba2addr(lba);
...
//        }
        if (written.find(lba) != written.end()) {
            ...
            if (it == data2log_map.end()) { // No log block mapped to it.
                if (empty_log_block >= (RAW_CAPACITY - BLOCK_SIZE)) {
                    ...
                    return logBlock->write(&address, lba);
                }
            } else {
            ...
            // The page is empty
            written.insert(lba);
            if (block_erases.find(data_block_id) == block_erases.end()) {
    ...
    std::pair<FlashSim::ExecState, FlashSim::Address> noEmptyPages(
        ...
        FlashSim::Address calculated_addr = lba2addr(lba);
        size_t data_block_id = lba / BLOCK_SIZE;
        FlashSim::Address cleaning_addr;

        if (getEraseNum(data_block_id) >= BLOCK_ERASES) {
//            new_data_block = true;
//            new_data_block_id = findEmptyDataBlock(data_block_id);
//            if (new_data_block_id >= ADDRESSABLE / BLOCK_SIZE) {
    ...
    std::pair<FlashSim::ExecState, FlashSim::Address> noEmptyLogBlocks(
        ...
        if (freed_log.size() > 0) {
            ...
            block_erases.insert(std::make_pair(chosen_log->block_id,
                                               min_erase+1));
            data2log_map.insert(std::make_pair(lba / BLOCK_SIZE, chosen_log));
            return chosen_log->write(&calculated_addr, lba);
        }
        ...
        data2log_map.insert(std::make_pair(lba / BLOCK_SIZE, log_block));
