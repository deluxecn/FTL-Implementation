  ...
  class Configuration {
   ...
   private:
    ...
    void ThrowFileNotFoundError() const {
      throw std::string{"The given configuration file "} + \
              file_name + \
              " could not be found!";
    }
    ...
    void ThrowKeyAlreadyExistsError(const std::string &key) const {
      throw "Key \"" + key + "\" already exists in the map";
    }
    ...
    void ThrowNoValueError(int line_num, const std::string line) const {
      throw std::string{"Configuration line "} + \
    ...
    void ThrowKeyDoesNotExistError(const std::string &key) const {
      throw std::string{"Key "} + key + " does not exist";
    }
   ...
   private:
    ...
    void ReadSlot(T *buffer, size_t slot_id) {
      ...
      if(it == active_slot_set.end()) {
        ThrowReadNonExistingSlotError(slot_id);
      }
    ...
    void EraseRange(size_t start_slot_id, size_t end_slot_id) {
      ...
      for(size_t slot_id = start_slot_id;slot_id <= end_slot_id;slot_id++) {
        ...
        if(it != active_slot_set.end()) {
          // As long as there is 1 valid slot the whole range
      ...
      // If all slots are inactive within the range then
      // we erased a fresh block, and this should be treated as an error
      if(all_inactive == true) {
        ThrowErasingInactiveRangeError(start_slot_id, end_slot_id);
      }
    ...
    void ThrowCreateTmpFileError() const {
      ...
      throw std::string{"Could not create temporary file! Errno = "} + \
            std::to_string(t);
    }
    ...
    void ThrowSparseFileNotSupportedError() const {
      throw std::string{"Sparse file is not supported"
    ...
    void ThrowSlotOutOfBoundError(size_t slot_id) const {
      throw std::string{"Slot "} + \
            std::to_string(slot_id) + \
            " is out of bound";
    }

    /*
     * ThrowErasingInactiveRangeError() - This is called when an inactive
     *                                    slot is erased
     */
    void ThrowErasingInactiveRangeError(size_t start_slot_id,
                                        size_t end_slot_id) const {
      throw std::string{"The range ["} + \
            std::to_string(start_slot_id) + \
            ',' + \
            std::to_string(end_slot_id) + \
            "] is entirely inactive (erase error)";
    }
    ...
    /*
     * ThrowReadNonExistingSlotError() - This is called when an inactive slot
     *                                   is being requested to read
     */
    void ThrowReadNonExistingSlotError(size_t slot_id) const {
      throw std::string{"Slot "} + \
            std::to_string(slot_id) + \
            " does not currently exist (read error)";
    }
  ...
  class Controller {
   ...
   private:
      ...
      switch(operation) {
        case OpCode::READ: {
          ...
          if(it == physical_logical_map.end()) {
            ...
            // in undefined data
            ThrowReadCleanPageError(physical_lba);
          }

          // If the read operation is correct then get its logical LBA
          size_t logical_lba = it->second;
    ...
    ExecState ReadLBA(PageType *page_p, size_t lba) {
      ...
      // If the data page read into the buffer is not associated with
      // the requested logical page, then this is an error
      if(page_buffer.front().second != lba) {
        ThrowInconsistentReadError(lba, page_buffer.front().second);
      }
    ...
    void ThrowStateNotCleanedError(size_t size) {
      throw std::string{"State not clean; buffer size = "} + \
            std::to_string(size);
    }
    ...
    void ThrowBlockDeadError(size_t block_lba) {
      throw std::string{"Block "} + \
            std::to_string(block_lba / block_size) + \
            " (linear block ID) has worn out";
    }

    /*
     * ThrowReadCleanPageError() - Read on a clean page
     */
    void ThrowReadCleanPageError(size_t physical_lba) {
      throw std::string{"Read operation on clean physical page "} + \
    ...
    void ThrowWriteDirtyPageError(size_t physical_lba) {
      throw std::string{"Write operation on dirty physical page "} + \
            std::to_string(physical_lba);
    }

    /*
     * ThrowInconsistentReadError() - Read returns a page that is not associated
     *                                with a correct logical page
     */
    void ThrowInconsistentReadError(size_t logical_lba, size_t physical_lba) {
      throw std::string{"Read inconsistent page. logical "} + \
            std::to_string(logical_lba) + \
            "; Physical " + \
            std::to_string(physical_lba);
    }
 ...
 public:
  ...
  int Write(FILE* log, size_t addr, const TEST_PAGE_TYPE& buf) {
    ...
    try {
      ...
      fprintf(log, "!!! Error writing LBA %zu !!!\n%s\n", addr, err.c_str());
      return -1;
    }
  ...
  int Read(FILE* log, size_t addr, TEST_PAGE_TYPE* buf) {
    ...
    try {
      ...
      fprintf(log, "!!! Error reading LBA %zu !!!\n%s\n", addr, err.c_str());
      return -1;
    }
    if (status != FlashSim::ExecState::SUCCESS) {
